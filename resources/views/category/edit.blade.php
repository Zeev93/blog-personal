            ...
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 bg-white border-b border-gray-200">
                    <div class="flex flex-col">
                        <h1 class="text-center text-3xl uppercase font-bold text-gray-700"> Edit Category </h1>
                    ...
                    <form action="{{ route('categories.update', ['category' => $category->id])  }}" method="POST" class="py-5">
                        @csrf
                        @method('PUT')
                        <label for="name" class="font-bold text-gray-700 uppercase">Category Name</label>
                        <input type="text" class="w-full block p-2 rounded shadow" name="name" id="name" value="{{ $category->name }}">
                        @error('name')
